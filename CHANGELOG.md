...
Codex 開新視窗常**等使用者先開口才動作**，不保證主動執行。

- **入口檔強化**（CLAUDE.md／AGENTS.md session-start 第1步）：明確「**回應第一則訊息前、主動（不必被問）
  讀 handoff**，並開場用一兩句說明上次到哪＋下一步」。對 Claude Code 尤其有效。
- **handoff 防覆蓋保護**：新增規則——**本 session 還沒做事就觸發寫入時，不准用空內容覆蓋既有 handoff，改成讀它**。
  防止新視窗誤把 `/handoff`（現預設為寫）變成空白覆蓋掉上一個 session 存好的交接。
- **使用建議**：新視窗要「讀」請用自然語言「接續上次／讀交接」，**別打 `/handoff`**（預設是寫）。
- ⚠️ 既有 Codex/Claude **需重開**載入更新後的入口檔與 handoff 指示。
