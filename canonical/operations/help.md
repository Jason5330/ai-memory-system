...
- **抓多久之前的？→ 抓「整段當前對話」**：範圍是**這次對話視窗裡還看得到的全部內容**（從開頭到現在），
  **不是固定時間窗**。所以你聊了很多輪、覺得有收穫，**隨時打 `/capture`** 就會把整段重點萃取成記憶，
  不是只看最近幾輪。
  - ⚠️ **超長對話會壓縮**：對話長到觸發 context 壓縮時，`/capture` 看到的是「摘要＋近期原文」，很早期的
    逐字細節可能已被濃縮——所以**覺得有收穫就趁早打**，原文還完整時細節保留最多。
  - 🔁 **已關掉的舊 session 不在範圍內**：那要用 **`/ingest-sessions`**（它讀硬碟上的 `.jsonl` 逐字稿，
    不靠記憶視窗）。`/capture`＝這次正在進行的對話；`/ingest-sessions`＝補抓過去的 session。
